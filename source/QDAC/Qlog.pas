...
resourcestring
  ...
  SPerfTagStopFormat = '[%s] 执行完成，用时 %d ms(%d->%d)';
...
begin
  ...
  begin
    FLazyMode := Value;
    if Value and Assigned(FCastor) then // 如果是懒人模式，则延迟100ms写入
...
begin
  FLastLazyWriteTime := {$IF RTLVersion>=23}TThread.{$IFEND} GetTickCount;
  FirstWriter;
...
begin
  ...
  if (LazyInterval > 0) then
  begin
    ADelta := {$IF RTLVersion>=23}TThread.{$IFEND} GetTickCount -
      FLastLazyWriteTime;
...
{$IF RTLVersion>=23}TThread.{$IFEND} GetTickCount;
  if FLogToConsole then
    ...
    PostLog(llDebug, PQCharW('  ' + PerfTagStopFormat), [FTag, ATick - FLastTick,
      FLastTick, ATick]);
  FLastTick := ATick;
end;
  ...
  begin
    ...
    begin
      if FItems is TStringList then
        ...
        // TListStrings一类的封装在大量记录时，如果直接操作，效率渣渣，不如用一个临时变量中转优化
