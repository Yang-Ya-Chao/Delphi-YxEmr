...
{$ENDIF}
  ...
  protected
    ...
    FMainThreadJobs: {$IFDEF UNICODE}Cardinal{$ELSE}Integer{$ENDIF};
...
function MakeJobProc(const AProc: TQJobProc): TMethod; overload;
var
  ATemp: TMethod absolute AProc;
begin
  Result := ATemp;
end;
...
begin
  ...
  begin
    ...
    try
      if FPlanCheckJob = 0 then
      // 再次检查，以避免在判断和锁定进入之前，别的线程已经执行过此段代码
      begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if FTimeoutCheck then
          Workers.Clear(Self);
        if Result in [wrTimeout, wrSignaled] then
          DoAfterDone;
      finally
