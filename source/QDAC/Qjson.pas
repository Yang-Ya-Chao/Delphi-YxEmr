...
{$IFEND >=2010}
    ...
    procedure SaveToStream(AStream: TStream; AEncoding: TTextEncoding = teUtf8;
      AWriteBom: Boolean = True; ADoFormat: Boolean = True); overload;
    /// <summary>保存当前对象内容到流中</summary>
    /// <param name="AStream">目标流对象</param>
    /// <param name="AEncoding">编码格式</param>
    /// <param name="AWriteBom">是否写入BOM</param>
    ...
    /// <remarks>注意当前结点的名称不会被写入</remarks>
    procedure SaveToStream(AStream: TStream; ASettings: TJsonEncodeSettings;
      AEncoding: TTextEncoding = teUtf8; AWriteBom: Boolean = True); overload;
    ...
    /// <remarks>注意当前结点的名称不会被写入</remarks>
    procedure SaveToFile(const AFileName: String;
      ASettings: TJsonEncodeSettings; AEncoding: TTextEncoding = teUtf8;
...
class function TQJson.JsonCat(const S: QStringW; ASettings: TJsonEncodeSettings)
  : QStringW;
var
  ABuilder: TQStringCatHelperW;
begin
...
class function TQJson.JsonEscape(const S: QStringW;
  ASettings: TJsonEncodeSettings): QStringW;
begin
...
procedure TQJson.SaveToFile(const AFileName: String;
  ASettings: TJsonEncodeSettings; AEncoding: TTextEncoding; AWriteBom: Boolean);
var
...
procedure TQJson.SaveToStream(AStream: TStream; ASettings: TJsonEncodeSettings;
  AEncoding: TTextEncoding; AWriteBom: Boolean);
var
  S: QStringW;
begin
  if DataType in [jdtArray, jdtObject] then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ((AFlags and MASK_OVERFLOW) <> 0) or (AFloat > 99999999999999)
        then
